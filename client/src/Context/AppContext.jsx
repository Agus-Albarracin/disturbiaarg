...
export const AppProvider = ({ children }) => {
    ...
    const fetchBtnnavLink = async () => {
        try {
            const response = await axios.get('http://localhost:4000/api/btnnav');
            const { data } = response;
    ...
    const updateBtnnavLink = async (newLink) => {
        try {
            const response = await axios.post('http://localhost:4000/api/btnnav', { link: newLink });
    ...
    const fetchImages = async () => {
        try {
            const response = await axios.get('http://localhost:4000/api/carousel');
    ...
    const addImage = async (imageUrl) => {
        try {
            const response = await axios.post('http://localhost:4000/api/carousel', { original: imageUrl });
    ...
    const removeImage = async (img_key) => {
        try {
            await axios.delete(`http://localhost:4000/api/carousel/${img_key}`);
    ...
    const fetchShippingPrices = async () => {
        try {
            const response = await axios.get('http://localhost:4000/api/shipping');
            const { data } = response
    ...
    const updateShippingPrices = async (newShippingPrices) => {
        try {
            await axios.post('http://localhost:4000/api/shipping', newShippingPrices);
    ...
    const updateFooterInfo = async (newInfo) => {
        try {
            await axios.post('http://localhost:4000/api/footer', newInfo);
